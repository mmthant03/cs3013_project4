...
void init()
{
	...
	{ //for each Process, malloc
		...
		for (int j = 0; j < 4; j++)
		{ //for each pageInMemory, set -1 for start
			tempProcess->isPageInMemory[j] = -1;
		}
		for (int j = 0; j < 4; j++)
		{ //for each PageTableEntry of each process, malloc
...
int storeFromStructToMemory()
{
	...
	{ //for each process
		...
		if (isPageTableInMemory != -1)
		{ //copy the page table to memory[] if needed
		...
		for (int k = 0; k < 4; k++)
		{ //load the pages
			if (currentProcess->isPageInMemory[k] != -1)
			{ //ifpage is in memory
				...
				for (int v = 0; v < 16; v++)
				{ //convert the ints to unsigned chars
...
int storeFromMemorytoStruct()
{
	...
	{ //for each process
		...
		if (isPageTableInMemory != -1)
		{ //use register values to get the page tables into active
